...
impl HtmlImageRule {
    ...
    fn create_img_tag(id: &NmdId, src: &str, label: &str) -> String {
        ...
        format!(r#"<figure class="figure" id="{}">
                ...
                </figure>"#, id.build(), src, label, label)
    }
}
...
impl ParsingRule for HtmlImageRule {
    ...
    fn parse(&self, content: &str, parsing_configuration: Arc<RwLock<ParsingConfiguration>>) -> Result<ParsingOutcome, ParsingError> {
        ...
        let mut parsing_configuration = parsing_configuration.read().unwrap();
                ...
                if let Some(src) = captures.get(2) {
                    ...
                    if RemoteResource::is_valid_remote_resource(src) {
                        ...
                        if parsing_configuration.embed_remote_image() {
                            ...
                            return Self::create_img_tag(&id, src.as_str(), label.as_str())
                        }
                        ...
                        if src_path_buf.exists() {
                            ...
                            return Self::create_img_tag(&id, format!("data:image/png;base64,{}", base64_image.unwrap()).as_str(), label.as_str());
                            ...
                            return Self::create_img_tag(&id, src, label.as_str())        // create image tag of invalid image instead of panic
                        }
...
mod test {
    ...
    fn parse_all_in_one() {
        ...
        let parsed_content = image_rule.parse(nmd_text.as_str(), Arc::new(RwLock::new(ParsingConfiguration::default()))).unwrap();
